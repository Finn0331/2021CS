...
[可用性(Availability)](https://ithelp.ithome.com.tw/articles/10156491)
...
[阿奇里斯腱(Achilles heel)](https://www.securitymagazine.com/articles/91431-the-new-security-achilles-heel)
 - 每個端點現在都是自己的邊界，這是一個允許攻擊者破壞網絡的弱點。一次錯誤的點擊或是下載，惡意程式都可以進行橫向傳播到整個組織。而使組織面臨風險的常見行為例如:插入公共充電站或使用不受信任的設備，包括其他計算機、閃存驅動器和 USB 端口或是在家裡、酒店或咖啡店工作時訪問不安全的 Wi-Fi 網絡或未能通過公司 VPN 進行身份驗證。上述行為本質上繞過了防火牆、IPS/IDS、VPN等網絡安全防禦。
