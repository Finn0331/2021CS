...
2. 小明在咖啡廳想存取家中電腦所提供的 FTP 服務，請問小明在出門前須先對管理家中電腦連線的防火牆訂定下列何種安全政策？A
(A) 開放連接埠 21，TCP 連線
(B) 開放連接埠 21，UDP 連線
(C) 開放連接埠 22，TCP 連線
(D) 開放連接埠 22，UDP 連線
- FTP屬於TCP服務
- 傳遞客戶端與伺服器之間的命令，一般設在 port 21
- 傳遞資料的，一般都設在 port 20
     


3. 防火牆限制外部網際網路使用者，只可存取放置組織公開資訊（對外 網站）的區域，不可進入內部網路，其放置組織公開資訊的區域一般 被稱為下列何者？C
...
4. 關於 Syn Flooding 網路阻斷服務攻擊，下列敘述何者「不」正確？B
(A) 用戶端發送 SYN 到伺服端
(B) 伺服器端回傳 ACK 到用戶端
(C) 用戶端不發送 ACK 到伺服端
...
5. 下列何者「不」是 TCP/IP 連線劫持（Session Hijacking）攻擊成功的必要前提？B
...
6. 網路應用程式防火牆（Web Application Firewall）用於防禦開放式系統互聯（Open System Interconnection, OSI）模型中第幾層之網路攻擊？D
(A) Layer 1
(B) Layer 3
(C) Layer 5
(D) Layer 7
...
10. 下列何者協定可能避免廣播風暴（Broadcast Storm）？A
...
11. 在資料進行傳輸過程中所使用的加密協定，下列何者為目前最安全者？C
(A) SSL 1.2
(B) SSL 3.0
(C) TLS 1.2
(D) TLS 3.0
...
12. TCP/IP 通訊協定中稱為網路介面層，負責與硬體溝通的是下列何者？C
...
13. 下列何者為 TCP 三向交握（Three-way Handshake）的最後步驟？A
(A) ACK
(B) SYN
(C) SYN/ACK
(D) ACK/SYN

14. 下列何種作業系統透過終端機或是工作站登入，可以供多人共同使用 並執行多個程式？B
(A) 多人單工系統
(B) 多人多工系統
(C) 單人單工系統
(D) 單人多工系統

15. 請問 grep -l -i pass /var/log/*.log 2＞/dev/null 指令，是要執行下列何種功能？A
...
- -i pass忽略大小寫
- *.log => 所有副檔名是log的檔案
- 2>/dev/null  => 把Error訊息重導向到/dev/null
- -l => 檔案裡面有包含pass的檔案名稱也包含pass的那一行


16. 某公司基於安全性考量，決定將目前的 802.11 無線網路由原先的 WPA2- Personal 認證方法變更為 WPA-Enterprise，請問管理員需要在網路環境中新增管理下列何種伺服器？B
(A) Kerberos 伺服器
(B) RADIUS 伺服器
(C) SNMP 伺服器
(D) NTP 伺服器
- 只有在連接RADIUS伺服器進行用戶端身份驗證時，才能使用WPA-Enterprise

17. 請問 cat ~/.bash_history 指令，是要執行下列何種功能？B
(A) 列出系統使用者
(B) 列出使用者曾經下過的指令
(C) 列出系統目錄
(D) 列出系統內的檔案
...
18. 應用程式執行特定作業結束後，並沒有通知作業系統，故無法向作業系統要求釋放記憶體空間，此狀況稱為下列何者？D
...
19. 系統管理人員於網站日誌中看見大量訊息含有類似字串 「admin」，請問可能為下列何種?A
...
21. M 公司授權進行內部網路的安全活動，相關人員於過程中使用 Nmap 進行掃描，發現有內部主機使用 vsftpd2.3.4，經判斷後發現具備漏洞 可利用，後續使用 Msfconsole 成功利用漏洞，此情境為下列何者？D
(A) 源碼掃描
(B) 社交工程
(C) 封包竊聽
(D) 滲透測試
...
22. 附圖為 OWASP Top 10 – 2017 文件敘述中的何項風險分類？C
(A) 跨網站指令碼（Cross-Site Scripting, XSS）
(B) XML 外部實體攻擊（XML External Entity Attack, XXE）
(C) 機敏資料外洩（Sensitive Data Exposure）
(D) 注入攻擊（Injection）

23. 下列何者是被 SQL 資料隱碼攻擊（SQL Injection）成功的原因？B
(A) 作業系統漏洞未即時更新修補程式（Patch）
(B) 未對使用者的輸入資料進行過濾與檢查
(C) 資料庫存取權限設定錯誤
(D) 遭受大量網路流量攻擊

24. 關於跨站指令碼攻擊（Cross-Site Scripting, XSS），請問攻擊成功的常見原因是資通系統未過濾或防範下列何種程式的注入攻擊？D
(A) Python
(B) ASP.NET
(C) ShellScript
(D) JavaScript

25. 下列何者是解決跨站請求偽造（Cross-Site Request Forgery, CSRF）的最佳方式？D
(A) 加入 HttpOnly
(B) 過濾特殊字元
(C) 使用 HTTPS 連線
(D) 使用圖形驗證碼
...
26. 使用 HTTP Cookie 的目的為下列何者？A
(A) 在瀏覽器中儲存資訊（如：Session ID）
(B) 作為瀏覽器的組態設定檔
(C) 防禦 XSS 攻擊
(D) 防禦 XML Injection 攻擊

27. 下列何者最「不」能防範 SQL 資料隱碼攻擊（SQL Injection）？A
(A) 對資料庫進行加密
(B) 對查詢字串進行字串過濾
(C) 使用 Prepare Statement
(D) 使用 Stored Procedure

28. 下列何者「不」是因為開發過程中，未留意程式安全造成的問題？A
...
29. 某資安人員想要更進一步了解與比較他在資料中心中利用掃描工具所發現的漏洞，探討每個弱點能否被遠端利用，或是攻擊者是否需要登入才能利用此弱點漏洞利用程度、修復級別與漏洞嚴重程度評分等資訊，請問他應該檢視漏洞報告內的何種指標資訊？D
(A) CSV
(B) NVD
(C) VSS
(D) CVSS
...
30. 若網頁瀏覽器的 Cookies 並未使用加密保護機制，網站設計者為圖登入方便性而將使用者帳密儲存在 Cookie 之中，此種安全漏洞可能讓駭 客使用下列何種網頁攻擊手法取得 Cookie 中的機敏資料？D
(A) SQL 資料隱碼攻擊（SQL Injection）
(B) XML 外部實體攻擊（XML External Entity Attack, XXE）
...
31. 下列何者「不」是發動中間人攻擊（Man-In-The-Middle, MITM）的必要手段？A
...
34. 關於資料傳輸安全（含實體資料及電子資料），下列敘述何者較「不」 正確？C
...
應在何處查看並確定哪種媒體存取控 制位址（Media Access Control Address, MAC）在特定時間具有特定的 IP 位址？C
(A) 在個人計算機的位址解析協定（Address Resolution Protocol, ARP） 緩存上
(B) 在 Web 服務器日誌文件中
(C) 在 DHCP 服務器日誌文件中
(D) 沒有辦法確定具體的 IP 地址

41. 關於日誌管理，下列敘述何者較正確？C
...
42. 事件紀錄檔（Event log）用於蒐集、統計和分析電腦生成的事件紀錄檔消息，對於了解複雜系統的活動軌跡至關重要，它是由用於生成、 傳輸、儲存、分析和處理事件紀錄檔數據的硬體、軟體、網路和媒體 所組成。關於事件紀錄檔，下列敘述何者較「不」正確？C
(A) 事件記錄包含伺服器登入資訊
(B) 事件記錄可以作為調查佐證
(C) 事件記錄包含工單資料
...
45. 關於雲端蜜罐（Honeypot），下列敘述何者「不」正確？A B
(A) 通常設置在正式的產品運作環境之中
(B) 任何連線蜜罐的行為都是可疑的
(C) 偽裝成有價值的網路或電腦系統，並設置漏洞，誘使駭客攻擊
(D) 可用來取得電腦病毒樣本

46. 關於行動裝置安全，下列何者「不」在保護的面向之中？A
(A) 擴充性
(B) 機密性
(C) 完整性
(D) 可用性

47. 關於行動裝置連線安全，下列敘述何者正確？C
...
48. 在物聯網裡，網路犯罪分子可能竊取用來加密通訊的金鑰，並將之用 於解譯加密過的資料，屬於下列何種攻擊手法？C
...
此一論點的主要見解為下列何者？B(1)企業應永遠假設自 身正在遭受攻擊、(2)企業應儘可能地降低攻擊所帶來的衝擊與影響、 
...
50. 攻擊者控制了物聯網其中一個節點，並丟棄（Drop）所有傳送至此節 點的封包，此為下列何種攻擊手法？D
(A) 黑函攻擊
(B) 分割攻擊
(C) 蟲洞攻擊
(D) 黑洞攻擊
